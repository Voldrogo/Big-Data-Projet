...
# --- Réentraîner le modèle (adapter les variables si besoin) ---
...
#cols_utilisees <- c("categorie_tarif", "puissance_nominale", "implantation_station",
 #         "condition_acces", "prise_type_2", "prise_type_combo_ccs",
  #        "prise_type_chademo", "paiement_cb", "reservation", "nbre_pdc")
...
fold_size <- floor(nrow(filtre_tarif_final) / 5)
resultats <- c()

for (i in 1:5) {
  idx_test <- ((i-1)*fold_size + 1):(i*fold_size)
  
  # restreindre aux colonnes utiles uniquement
  train <- filtre_tarif_final[-idx_test, cols_utilisees]
...
# --- 2. Probabilités prédites selon la puissance ---# --- 2. Probabilités prédites selon la puissance ---
grid_pred <- data.frame(
  puissance_nominale   = seq(min(filtre_tarif_final$puissance_nominale),
                             max(filtre_tarif_final$puissance_nominale), length.out = 100),
  implantation_station = filtre_tarif_final$implantation_station[1],
  condition_acces      = filtre_tarif_final$condition_acces[1],
  prise_type_2         = filtre_tarif_final$prise_type_2[1],
  prise_type_combo_ccs = filtre_tarif_final$prise_type_combo_ccs[1],
  prise_type_chademo   = filtre_tarif_final$prise_type_chademo[1],
  paiement_cb          = filtre_tarif_final$paiement_cb[1],
  reservation          = filtre_tarif_final$reservation[1],
  nbre_pdc             = median(filtre_tarif_final$nbre_pdc)
)

probs <- predict(modele, newdata = grid_pred, type = "probs") %>%
  as.data.frame() %>%
  bind_cols(puissance = grid_pred$puissance_nominale) %>%
  pivot_longer(-puissance, names_to = "categorie", values_to = "prob")

ggplot(probs, aes(x = puissance, y = prob, color = categorie, fill = categorie)) +
  geom_area(alpha = 0.2, position = "identity") +
  geom_line(linewidth = 1) +
  scale_y_continuous(labels = scales::percent) +
  labs(x = "Puissance nominale (kW)", y = "Probabilité prédite",
       title = "Probabilités prédites selon la puissance", color = NULL, fill = NULL) +
  theme_minimal()
